                  ...
                  {/* {getAirport === "DXB" ? (
                    <NetworkInternationalPay
                      ...
                      onPaymentSuccess={(result) => {
                        console.log("✅ Network International payment successful:", result); 
                        dispatch(setPaymentSuccess(result));
                      }}
                      onPaymentError={(error) => {
                        console.error("❌ Network International payment failed:", error);
                        dispatch(
                          setPaymentError(error.message || "Payment failed")
                        );
                      }}
                      // Add the missing props for booking sync
                      onBookingSync={triggerSync} // Function from useBookingSync hook  
                      multiModeReference={multimode} // Multi mode reference from useBookingSync  
                      referenceNo={referenceNo} // Reference numbers from useBookingSync  
                      supplierCost={correctPricing.supplierCost || 0} // Calculate supplier cost  
                      syncStatus={syncStatus}
                    /> 
                  ) : ( */}
                    <StripePay
                      ...
                      referenceNo={referenceNo} // Reference numbers from useBookingSync
                      supplierCost={correctPricing.supplierCost || 0} // Calculate supplier cost
                      syncStatus={syncStatus} // Booking sync status
                    />
                  {/* )} */}
